...
**One deliberate deviation from the spec**, flagged rather than silent: the spec put `@hyperframes/*` in `devDependencies`. That would download Chromium (via Puppeteer) on every Vercel build, since Vercel installs devDependencies. The plan keeps HyperFrames out of `package.json` and calls it through `npx`. **The spec was amended to match, in commit `5e04d16`.**
